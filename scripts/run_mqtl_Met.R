...
met_matrix_name <-paste("/hps/nobackup/ma/natalja/data/mQTL_data/met_matrices/",base,diagnosis,"_Met.txt",sep="")
cov_name <- paste("/hps/nobackup/ma/natalja/data/mQTL_data/covariates/Covariates_",base,diagnosis,".txt",sep="") 
output_file_name <- paste("/hps/nobackup/ma/natalja/data/mQTL_data/results/mqtl_",base,diagnosis,"_Met.txt",sep="") 
  
#mQTL Analysis
mqtl_analysis(snps,met_matrix_name,cov_name,output_file_name)

permuted_met_matrix_name <- paste(gsub(".tsv","",met_matrix_name),"_permuted_Met.tsv",sep="")
permute_matrix(met_matrix_name,permuted_met_matrix_name)
permuted_output_file_name <- paste("/hps/nobackup/ma/natalja/data/mQTL_data/results/mqtl_permuted_",base,diagnosis,"_Met.txt",sep="") 
...
if (nrow(res_permuted[res_permuted$FDR<0.01,])==0){
    ...
    write.table(res,paste("/hps/nobackup/ma/natalja/data/mQTL_data/results/result_",base,diagnosis,"_annot_Met.txt",sep="") ,sep="\t",row.names = FALSE)
    write.table(res_annot,paste("/hps/nobackup/ma/natalja/data/mQTL_data/results/result_",base,diagnosis,"_annot_full_Met.txt",sep=""),sep="\t",row.names = FALSE)
}
