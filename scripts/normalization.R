################################################################################
# Metabolomic data normalization (EigenMS, QN, ComBat)
# EigenMS
# "Normalization of peak intensities in bottom-up MS-based proteomics using
...
for(base in bases){
  ...
  if (base =="NMR_CPMG_Serum"){
    ...
    batch = cov$Centre
    modcombat = model.matrix(~1, data=cov)
    combat_data = ComBat(dat=ddata, batch=batch, mod=modcombat, par.prior=FALSE, prior.plots=TRUE)
    ddata <- combat_data
...
for(base in bases){
  ...
  file_name <- paste(main_dir,base,"_original.txt",sep="")
  output_file_name <- paste("/home/natalja/Documents/EMIF/KCL/NPC/NPC/NMR/normalized_data/",base,"_EigenMS.tsv",sep="")
  qn_output_file_name <- paste("/home/natalja/Documents/EMIF/KCL/NPC/NPC/NMR/normalized_data/",base,"_EigenMS_QN.tsv",sep="")
